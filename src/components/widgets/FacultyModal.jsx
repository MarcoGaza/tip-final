...
function OrgChartComponent({ facultyData }) {
  ...
  useEffect(() => {
    ...
    const chart = new OrgChart()
      ...
      .nodeContent((d, i, arr, state) => {
        return `
          <div class="w-[${d.width}px] h-[${d.height}px]" onclick="handleCardClick(${JSON.stringify(d.data)})">
            <div class="p-2.5 border border-gray-300 rounded-md flex flex-col items-center">
  ...
  return (
    <div>
      ...
      {selectedMember && (
        <FacultyModal member={selectedMember} onClose={handleCloseModal} />
      )}
    </div>
  );
}
